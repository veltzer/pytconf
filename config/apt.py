...
apt_arch = subprocess.check_output('dpkg-architecture | grep -e ^DEB_BUILD_ARCH= | cut -d = -f 2',
                                   shell=True).decode().rstrip()
apt_architectures = '{0} source'.format(apt_arch)
apt_component = 'main'
apt_folder = 'apt'
apt_service_dir = os.path.join(config.general.general_homedir, 'public_html/public', apt_folder)
apt_except = '50{0}'.format(personal_slug)
apt_pack_list = glob.glob(os.path.join(config.general.general_homedir, 'packages', '*.deb'))
